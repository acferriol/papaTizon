...
class ResumenSemanal(TemplateView, UserPassesTestMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        variedad = request.user.empresa.variedad_actual
        defEnd = datetime.today()
        # print(defEnd)
        defStart = defEnd - timedelta(days=6)
        week = (
            Clima.objects.all()
            .filter(empresa=idEmp)
            .filter(fecha__gte=defStart)
            .filter(fecha__lte=defEnd)
        )
        num_days = len(week)
        sev_acum = 0
        favs = 0
        ...
        senal = buscar_senal(sev_acum, favs, variedad)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if seleccion != "Seleccionar una empresa":
            emp = int(seleccion)
            obj_emp = Empresa.objects.values("nombre", "variedad_actual").filter(id=emp)
            nomb_emp = obj_emp[0]["nombre"]
            variedad = obj_emp[0]["variedad_actual"]
            week = (
                Clima.objects.all()
                .filter(empresa=emp)
