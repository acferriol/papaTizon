...
def clasificar_dia(id):
    ...
    if nwclima.temperatura_maxima == None or nwclima.temperatura_media == None or nwclima.temperatura_minima == None or nwclima.precipitacion == None:
        nwclima.favorable = -1
    else:
        #Ultimos 6 dias validos
        six_days = Clima.objects.exclude(
           temperatura_maxima = None, 
            temperatura_media = None,
            temperatura_minima =None,
            precipitacion = None 
        ).order_by("fecha").reverse()[1:7]

        five_days = Clima.objects.exclude(
           temperatura_maxima = None, 
            temperatura_media = None,
            temperatura_minima =None,
            precipitacion = None 
        ).order_by("fecha").reverse()[1:6]
        ...
        if len(six_days) < 6:
            ...
            if not dif:
                ...
                total_prec = six_days.aggregate(Sum('precipitacion'))['precipitacion__sum']
                ...
                avg_tm = five_days.aggregate(Avg('temperatura_media'))['temperatura_media__avg']
    ...
    if len(six_days) < 6:
        ...
        if not dif or (nwclima.fecha-six_days[1].fecha).days < 0:
            ...
            if six_days[1].deteccion_inicial > 0:
                nwclima.deteccion_inicial = six_days[1].deteccion_inicial + (nwclima.fecha-six_days[1].fecha).days
            else:
                sev = 0
                favs = 0
...
def tipo_diagnostico():
    ...
    return Clima.objects.filter(fecha__gt=start).filter(fecha__lt=end).filter(deteccion_inicial__gt=1).exists()