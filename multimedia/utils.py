...
def contextGraph(datos):
    ...
    variedad = Empresa.objects.values("variedad_actual").filter(id=datos["empresa"])
    clima = (
        Clima.objects.values(
            "fecha",
            "estacion",
            "temperatura_media",
            "temperatura_maxima",
        ...
        .filter(fecha__gte=start)
        .filter(fecha__lte=end)
        .filter(estacion=est[0]["estacion"])
        .order_by("fecha")
    )

    ####PRIMER GRAFICO####
    tmax = [
        (clima[i]["fecha"], clima[i]["temperatura_maxima"])
        for i in range(len(clima))
        if clima[i]["temperatura_maxima"] is not None
    ]
    tmed = [
        (clima[i]["fecha"], clima[i]["temperatura_media"])
        for i in range(len(clima))
        if clima[i]["temperatura_media"] is not None
    ]
    tmin = [
        (clima[i]["fecha"], clima[i]["temperatura_minima"])
        for i in range(len(clima))
        if clima[i]["temperatura_minima"] is not None
    ]
    prec = [
        (clima[i]["fecha"], clima[i]["precipitacion"])
        for i in range(len(clima))
        if clima[i]["precipitacion"] is not None
    ]
    ...
    clima = (
        Clima.objects.values(
            "fecha", "estacion", "favorable", "severidad", "deteccion_inicial"
        )
        .filter(fecha__gte=start)
        .filter(fecha__lte=end)
        .filter(estacion=est[0]["estacion"])
    ...
    for i in range(len(clima)):
        ...
        if clima[i]["deteccion_inicial"] == 0:
            ...
            if clima[i]["deteccion_inicial"] == 1:
                ...
                sign = buscar_senal(sev_acum, fav, variedad)
